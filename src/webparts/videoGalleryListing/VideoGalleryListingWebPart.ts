...
// import styles from './VideoGalleryListingWebPart.module.scss';
import * as strings from 'VideoGalleryListingWebPartStrings';

import { SPHttpClient, SPHttpClientResponse } from '@microsoft/sp-http';
import DmccVideoGalleryListing from './DmccVideoGalleryListing';

export interface IVideoGalleryListingWebPartProps {
  description: string;
}
...
export default class VideoGalleryListingWebPart extends BaseClientSideWebPart<IVideoGalleryListingWebPartProps> {
  ...
  private async _renderListAsync(apiUrl: string): Promise<any> {
    await this._getListData(apiUrl)
      .then((response) => {
          this._renderVideoGallery(response.value);
      });
  }
  ...
  private _renderVideoGallery(items: ISPList[]): void {
    ...
    items.forEach((item: ISPList) => {
      ...
      singleElementHtml = singleElementHtml.replace("#VDOTITLE", videoTitle + "");
      allElementsHtml += singleElementHtml;
    });

    if (allElementsHtml == "") { allElementsHtml = DmccVideoGalleryListing.noRecord; }
            ...
            <div class="col-box-wrapper w-100 float-start bg-white gp-shadow">
              <div class="col-box-title d-flex bg-white position-relative">
                <div class="col-box-icon sqbx-theme-6 d-flex flex-shrink-0 align-items-center justify-content-center">
                  <img class="mw-px-60" src="//dmccdxb.sharepoint.com${this._FirstSite}/SiteAssets/images/icons/image-gallery-icon.png" />
                </div>
              ...
              <div id="divVideoGalAllElements" class="row gap-0 px-4 py-4 gy-4">
