...
// import styles from './NewJoineeListingWebPart.module.scss';
import * as strings from 'NewJoineeListingWebPartStrings';

import { SPHttpClient, SPHttpClientResponse } from '@microsoft/sp-http';
import DmccNewJoinees from './DmccNewJoinees';

export interface INewJoineeListingWebPartProps {
  description: string;
}
...
export default class NewJoineeListingWebPart extends BaseClientSideWebPart<INewJoineeListingWebPartProps> {
  ...
  private async _renderListAsync(apiUrl: string): Promise<any> {
    await this._getListData(apiUrl)
      .then((response) => {
          this._renderNewJoinee(response.value);
      });
  }
  ...
  private _renderNewJoinee(items: ISPList[]): void {
    ...
    items.forEach((item: ISPList) => {
      let singleElementHtml: string = DmccNewJoinees.newJoineeSingleElement;
    ...
    if (allElementsHtml == "") { allElementsHtml = DmccNewJoinees.noRecord; }

    const divVideoGalAllElements: Element | null = this.domElement.querySelector('#NewJoineeListingDiv');
        ...
        <div class="row">
          <div class="col-12 mb-4">
            <div class="col-box-wrapper w-100 float-start bg-white gp-shadow d-flex flex-column">
              ...
              <div id="NewJoineeListingDiv" class="row gap-0 px-4 py-4 gy-4">
  ...
  let apiUrl = `${this.context.pageContext.web.absoluteUrl}/_api/web/lists/GetByTitle('NewJoinees')/items?$select=ID,EmployeeName/Title,EmployeeImage,Position,Department,ReadMoreLink&$expand=EmployeeName&$filter=IsActive eq 1&$orderby=Modified%20desc`;
